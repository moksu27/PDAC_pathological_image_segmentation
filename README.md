...
It is recommended to use the **QuPath** program to generate patch data for pathological images. Please refer to the QuPath_WSI_to_Patch.groovy file for generating patch data using QuPath.
...
It is recommended to use the **PyHIST** library to generate patch data and then use it for visualization.
...
model 학습을 위한 input data의 경우 patch data이어야만 하며 pathological image에 대한 patch data는 QuPath program을 이용하여 생성하는 것을 권장합니다.
...
Visualizatino input data의 경우 각 패치 데이터의 좌표 값이 필요하기 때문에 **PyHIST** 라이브러리를 사용하여 패치 데이터 생성 후 사용하시길 권장합니다.
