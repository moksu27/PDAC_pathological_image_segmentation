...
class SemiDataset(Dataset):
    def __init__(self, img_root, mask_root, mode, size=None):
    ...
    def __getitem__(self, i):
        ...
        data_w = weak_transform(image=img_w, mask=None)
        img_w = data_w["image"]

        data_s1 = strong_transform(image=img_s1, mask=mask)
        img_s1 = data_s1["image"]
        cutmix_box1 = obtain_cutmix_box(CFG["IMG_SIZE"], p=0.5)

        data_s2 = strong_transform(image=img_s2, mask=mask)
